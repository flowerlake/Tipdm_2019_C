...
def train():
  ...
  with tf.Graph().as_default():
	...
	h_1 = tf.tanh(tf.nn.conv2d(output_feature_1,h_matrix_1,strides=[1,1,1,1], padding='SAME') + h_bias_1)
      ...
      G_predicts = tf.sigmoid(G_model.logits)
    ...
    saver_load.restore(sess,"/scratch/ys1297/LSTM_tracking/source/checkpoints/test/model.ckpt-28400")
    for step in xrange(FLAGS.max_step):
      t= time.time()
      feature_0_mask,feature_0_no_mask, all_features, masks,all_images = data_handler.GetBatch() # first frame masked feature,all feature from all frames, masks for all frames,       mask_feature:(batch_size,14,14,512), all_features:(batch_size,length,14,14,512),  masks:(batch_size,length,14,14,512)
      ...
      g_predict = sess.run(G_predicts,feed_dict= G_feed_dict) 
      elapsed = time.time() - t

      print("time per batch is " + str(elapsed))
      print(step)
      data_handler.generate_overlay_image(all_images[0,:,:,:,:],masks[0,:,:,:,:],'./truth_train'+str(step)) 
      data_handler.generate_overlay_image(all_images[0,:,:,:,:],g_predict[0,:,:,:,:],'./predict_train'+str(step))

      #scipy_io.savemat('predict_test',{'g_predicts':g_predict,'ground_truth':masks,'all_features':all_features})
      if step ==10:
        sys.exit()	
      #if step %100==0:
